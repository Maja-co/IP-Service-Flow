...
public class ServiceOpgaveService {
    ...
    private List<ServiceInterval> GetServiceIntervaller() =>
    ...
    private async Task<List<Maskine>> GetMaskinerByKundeIdAsync(int kundeId) =>
        await _context.Maskiner.Where(m => m.KundeId == kundeId).ToListAsync();

    private async Task<List<ServiceTeknikker>> GetAlleTeknikereAsync() =>
    ...
    public async Task OpretOpgaveFraUiAsync(
        ...
        var maskine = await _context.Maskiner.FindAsync(maskineId.Value) ??
                      throw new Exception("Maskinen findes ikke i databasen.");
        var medarbejder = await _context.Medarbejdere.FindAsync(medarbejderId) ??
                          throw new Exception("Medarbejderen findes ikke i databasen.");
        ...
        if (kategori == "Sikkerhedseftersyn") {
            ...
            nyOpgave = new SikkerhedsEftersyn(maskine, regelListe, sidstUdført, deadlineDateOnly, note, interval.Value,
                medarbejder, tekniker!);
        }
            ...
            nyOpgave = new Service(maskine, serviceType.Value, opgaveTypeListe, sidstUdført, deadlineDateOnly, note,
    ...
    public async Task OpdaterOpgaveFraUiAsync() {
        ...
        opgave.ServiceInterval = ValgtInterval!.Value; 
        ...
        if (opgave is SikkerhedsEftersyn eftersyn && ValgtKategori == "Sikkerhedseftersyn" &&
            ValgtEftersynsRegelId.HasValue) {
        ...
        else if (opgave is Service service && ValgtKategori == "Service" && ValgtServiceType.HasValue &&
                 ValgtOpgaveTypeId.HasValue) {
